...
from src.config import CSV_PATH_1, CSV_PATH_2, CSV_PATH_3
...
class Item:
    ...
    @classmethod
    def instantiate_from_csv(cls, csv_path=CSV_PATH_3):
        """Инициализирует экземпляры класса Item данными из CSV-файла
        (при отсутствии или повреждении файла вызывает исключения)"""
        try:
            if not os.path.exists(csv_path):
                raise FileNotFoundError(f'Отсутствует файл {csv_path.split("/")[-1]}')

            cls.all.clear()
            with open(csv_path, 'r', encoding='windows-1251') as file:
                reader = csv.DictReader(file)
                for i in reader:
                    if set(i) != {'name', 'price', 'quantity'}:
                        raise InstantiateCSVError(csv_path)
